...
```
```{python, include = FALSE}
# Let's define a simple CNN to classify photod
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if i % 5000 == 0:
            with torch.no_grad():
                try:
                    # Show a sample of images from the tran data and model's predictions
...
# Store a batch of images, actual labels, and predicted labels
...
```{python}
# Select a subset of images, actual labels, and predicted labels for display
display_images = torch.cat(display_images)[:8]
actual_labels = actual_labels[:8]
predicted_labels = predicted_labels[:8]

# Display selected images along with their actual and predicted labels
imshow(torchvision.utils.make_grid(display_images))
print('Actual Labels:', ' '.join('%5s' % label for label in actual_labels))
print('Predicted Labels:', ' '.join('%5s' % label for label in predicted_labels))
```
...
train_images, train_labels = next(trainiter)
trainiter = iter(test_loader)
test_images, test_labels = next(trainiter)
train_images, train_labels = train_images.to(device), train_labels.to(device)
